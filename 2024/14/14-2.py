...
def longest_group_length(strip):
    active = False
    best_len = 0
    cur_len = 0
    ...
    for b in strip:
        if b:
            cur_len += 1
        if active and not b:
            best_len = max(cur_len,best_len)
            cur_len = 0
        active = b
    
    return max(cur_len, best_len)

def christmas_tree_check(robots):
    robots_at_position = [[False for _ in range(COLUMNS)] for _ in range(ROWS)]
    for robot in robots:
        robots_at_position[robot.r][robot.c] = True

    for row in robots_at_position:
        # Look for bottom of tree
        if longest_group_length(row) > 15:
            return True
    return False
...
for i in range(10000):
    ...
    if christmas_tree_check(robots):
        print_robots(robots)
        print("CHRISTMAS TREE at !")
        print("Seconds", i+1)
        break
